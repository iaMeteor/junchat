...
public extension Bundle {
    ...
    static let junchatSimplifiedChineseLocalization = "zh-Hans"
    static let junchatTraditionalChineseLocalization = "zh-Hant-TW"
    
    static var junchatPreferredLocalizations: [String] {
        let preferredLanguages = overrideLocalizations ?? Locale.preferredLanguages
        let localization = preferredLanguages.compactMap(junchatSupportedLocalization(for:)).first ?? junchatSimplifiedChineseLocalization
        return [localization]
    }
    
    static var junchatElementCallLanguage: String {
        junchatPreferredLocalizations.first == junchatTraditionalChineseLocalization ? "zh-Hant" : "zh-Hans"
    }
    ...
    private static func junchatSupportedLocalization(for language: String) -> String? {
        let normalizedLanguage = language.replacingOccurrences(of: "_", with: "-").lowercased()
        
        guard normalizedLanguage.hasPrefix("zh") else {
            return nil
        }
        
        if normalizedLanguage.contains("hant") ||
            normalizedLanguage.contains("-tw") ||
            normalizedLanguage.contains("-hk") ||
            normalizedLanguage.contains("-mo") {
            return junchatTraditionalChineseLocalization
        }
        
        return junchatSimplifiedChineseLocalization
    }
}
