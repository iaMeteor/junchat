...
class AppCoordinator: AppCoordinatorProtocol, AuthenticationFlowCoordinatorDelegate, NotificationManagerDelegate, SecureWindowManagerDelegate {
    ...
    init(appDelegate: AppDelegate) {
        ...
        if let parsedVersion = Self.migrationVersion(from: rawCurrentVersion) {
            currentVersion = parsedVersion
        } else {
            MXLog.error("Unable to parse app version \(rawCurrentVersion) for migrations.")
            currentVersion = Version(0, 0, 0)
        }
        
        if let rawPreviousVersion = appSettings.lastVersionLaunched {
            if let previousVersion = Self.migrationVersion(from: rawPreviousVersion) {
                performMigrationsIfNecessary(from: previousVersion, to: currentVersion)
            } else {
                MXLog.error("Unable to parse previous app version \(rawPreviousVersion) for migrations.")
            }
        } else {
    ...
    static func migrationVersion(from rawVersion: String) -> Version? {
        if let version = Version(rawVersion) {
            return version
        }
    ...
    #if DEBUG
    private var debugStartupRoute: AppRoute? {
        let environment = ProcessInfo.processInfo.environment
        
        if let roomID = environment["JUNCHAT_DEBUG_CALL_ROOM_ID"], !roomID.isEmpty {
            let voiceOnly = environment["JUNCHAT_DEBUG_CALL_VOICE_ONLY"].map { $0 != "false" } ?? true
            return .call(roomID: roomID, isVoiceCall: voiceOnly)
        }
        
        if let roomID = environment["JUNCHAT_DEBUG_ROOM_ID"], !roomID.isEmpty {
            return .room(roomID: roomID, via: [])
        }
    ...
    private func observeAppLockChanges() {
        appLockFlowCoordinator.actions.sink { [weak self] action in
            ...
            case .unlockApp:
                windowManager.switchToMain()
            case .disableAccountAndForceLogout:
                Task {
                    if let userSession = self.userSession {
                        _ = await userSession.clientProxy.disableOwnAccountForEmergencyPIN()
                    }
                    self.stateMachine.processEvent(.signOut(isSoft: false, disableAppLock: true))
                }
            case .forceLogout:
                stateMachine.processEvent(.signOut(isSoft: false, disableAppLock: true))
            }
