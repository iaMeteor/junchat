...
@Observable class NavigationTabCoordinator<Tag: Hashable>: CoordinatorProtocol, CustomStringConvertible {
    ...
    func setTabs(_ tabs: [Tab], animated: Bool = true) {
        ...
        let currentTabModules = tabModules
        withTransaction(transaction) {
            tabModules = tabs.map { tab in
                if let existingModule = currentTabModules.first(where: { $0.details.tag == tab.details.tag && $0.coordinator === tab.coordinator }) {
    ...
    var overlayAllowsHitTesting: Bool {
    ...
    func setOverlayCoordinator(_ coordinator: (any CoordinatorProtocol)?,
        ...
        let previousOverlayModule = overlayModule

        guard let coordinator else {
            previousOverlayModule?.tearDown()
            overlayModule = nil
            return
        }
            ...
            Group {
                if let coordinator = navigationTabCoordinator.overlayModule?.coordinator {
                    coordinator.toPresentable()
                        ...
                        .allowsHitTesting(navigationTabCoordinator.overlayAllowsHitTesting)
                        .transition(.opacity)
                }
            }
