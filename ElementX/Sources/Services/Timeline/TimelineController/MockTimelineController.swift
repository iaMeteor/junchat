...
class MockTimelineController: TimelineControllerProtocol {
    ...
    private(set) var redactedEventOrTransactionID: TimelineItemIdentifier.EventOrTransactionID?
    private(set) var redactedEventOrTransactionIDs = [TimelineItemIdentifier.EventOrTransactionID]()
    ...
    func sendMessage(_ message: String,
                     ...
                     intentionalMentions: IntentionalMentions) async {
        let item = TextRoomTimelineItem(id: .event(uniqueID: .init(UUID().uuidString),
                                                   eventOrTransactionID: .eventID("mock-sent-message-\(timelineItems.count + 1)")),
                                        timestamp: .now,
                                        isOutgoing: true,
                                        isEditable: true,
                                        canBeRepliedTo: true,
