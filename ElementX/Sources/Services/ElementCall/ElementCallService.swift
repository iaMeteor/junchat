...
class ElementCallService: NSObject, ElementCallServiceProtocol, @preconcurrency PKPushRegistryDelegate, @preconcurrency CXProviderDelegate {
    ...
    func setupCallSession(roomID: String,
        ...
        // An active CallKit session owns media in the app process. Element Call captures media in
        // WebKit's process, so keeping CallKit active prevents the remote participant hearing us.
        // Incoming calls use CallKit only until answer; outgoing web calls must not start one.
        _ = roomDisplayName
    }
    ...
    func acceptIncomingCall(roomID: String,
        ...
        guard answerCallHandoffIdentity != trackedIncomingCallIdentity else {
            MXLog.info("Ignoring in-app accept while the matching CallKit answer is in progress")
            return nil
        }
        ...
        MXLog.info("[JunchatCall] acceptIncomingCall releasing CallKit media ownership")
        reportedIncomingCallKitIDs.remove(incomingCallID.callKitID)
        activeCallKitIDs.remove(incomingCallID.callKitID)
    ...
    func setAudioEnabled(_ enabled: Bool, roomID: String) {
        ...
        guard activeCallKitIDs.contains(ongoingCallID.callKitID) else { return }
    ...
    func provider(_ provider: CXProvider, perform action: CXAnswerCallAction) {
        ...
        guard answerCallHandoffIdentity != incomingCallIdentity else {
            MXLog.info("Ignoring duplicate CallKit answer while its handoff is in progress")
            return
        }
    ...
    private func cancelAnswerCallHandoff() {
        answerCallHandoffIdentity = nil
