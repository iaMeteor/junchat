...
class ClientProxy: ClientProxyProtocol {
    ...
    func disableOwnAccountForEmergencyPIN() async -> Result<Void, ClientProxyError> {
        do {
            let session = try client.session()
            let url = appSettings.pushGatewayBaseURL
                .appending(path: "account")
                .appending(path: "disable-self")
            var request = URLRequest(url: url)
            ...
            guard let httpResponse = response as? HTTPURLResponse,
                ...
                MXLog.error("Failed disabling account with emergency PIN: invalid response \(response)")
            ...
            MXLog.error("Failed disabling account with emergency PIN: \(error)")
            return .failure(.sdkError(error))
        }
    }
    ...
    func changePassword(oldPassword: String, newPassword: String, logoutDevices: Bool) async -> Result<Void, ClientProxyError> {
        do {
            let session = try client.session()
            let url = try junchatPasswordChangeURL(session: session)
            let auth = JunchatPasswordChangeAuth(userID: userID, password: oldPassword)
            
            let firstResponse = try await submitPasswordChange(url: url,
                                                               accessToken: session.accessToken,
                                                               body: JunchatPasswordChangeRequest(newPassword: newPassword,
                                                                                                  logoutDevices: logoutDevices,
                                                                                                  auth: auth))
            if 200..<300 ~= firstResponse.statusCode {
                return .success(())
            }
            
            if firstResponse.statusCode == 401,
               let uiaSession = firstResponse.uiaSession {
                var retryAuth = auth
                retryAuth.session = uiaSession
                let retryResponse = try await submitPasswordChange(url: url,
                                                                   accessToken: session.accessToken,
                                                                   body: JunchatPasswordChangeRequest(newPassword: newPassword,
                                                                                                      logoutDevices: logoutDevices,
                                                                                                      auth: retryAuth))
                if 200..<300 ~= retryResponse.statusCode {
                    return .success(())
                }
            ...
            MXLog.error("Failed changing account password: HTTP \(firstResponse.statusCode)")
            return .failure(.invalidResponse)
        } catch let error as ClientProxyError {
            return .failure(error)
        } catch {
            MXLog.error("Failed changing account password: \(error)")
            return .failure(.sdkError(error))
        }
    }
    ...
    private func submitPasswordChange(url: URL,
                                      accessToken: String,
                                      body: JunchatPasswordChangeRequest) async throws -> JunchatPasswordChangeResponse {
        var request = URLRequest(url: url)
        request.httpMethod = "POST"
