...
class SettingsScreenViewModel: SettingsScreenViewModelType, SettingsScreenViewModelProtocol {
    ...
    private var contactsDirectoryVisibilityTask: Task<Void, Never>?
    private var hasChangedContactsDirectoryVisibility = false
    ...
    init(userSession: UserSessionProtocol, appSettings: AppSettings, isBugReportServiceEnabled: Bool) {
        ...
        super.init(initialViewState: .init(deviceID: userSession.clientProxy.deviceID,
                                           ...
                                           showEntertainmentTab: appSettings.showEntertainmentTab,
                                           selectedCallRingtone: appSettings.callRingtone,
                                           selectedMessageNotificationSound: appSettings.messageNotificationSound,
        ...
        appSettings.$showEntertainmentTab
            .weakAssign(to: \.state.showEntertainmentTab, on: self)
            .store(in: &cancellables)

        appSettings.$callRingtoneSoundName
            .map { JunchatCallRingtone.ringtone(for: $0) }
            .weakAssign(to: \.state.selectedCallRingtone, on: self)
            .store(in: &cancellables)
        ...
        Task {
            ...
            await loadContactsDirectoryVisibility()
        }
    }
    ...
    override func process(viewAction: SettingsScreenViewAction) {
        ...
        case .junchatSupport:
            actionsSubject.send(.junchatSupport)
        case .changePassword:
            actionsSubject.send(.changePassword)
        case .callRingtoneSettings:
            actionsSubject.send(.callRingtoneSettings)
        case .messageNotificationSoundSettings:
            actionsSubject.send(.messageNotificationSoundSettings)
        ...
        case let .updateHideFromContactsDirectory(value):
            contactsDirectoryVisibilityTask = Task { [weak self] in
                await self?.updateHideFromContactsDirectory(value)
            }
        case let .updateShowEntertainmentTab(value):
            appSettings.showEntertainmentTab = value
            state.showEntertainmentTab = value
        case let .updateCallRingtone(ringtone):
            appSettings.callRingtone = ringtone
            state.selectedCallRingtone = ringtone
        case let .updateMessageNotificationSound(sound):
            appSettings.messageNotificationSound = sound
            state.selectedMessageNotificationSound = sound
        }
    }

    private func loadContactsDirectoryVisibility() async {
        guard case .success(let hidden) = await clientProxy.junchatHideFromContactsDirectory() else {
            return
        }

        guard !hasChangedContactsDirectoryVisibility else {
            return
        }

        state.hideFromContactsDirectory = hidden
    }

    private func updateHideFromContactsDirectory(_ hidden: Bool) async {
        defer {
            contactsDirectoryVisibilityTask = nil
            state.isWaitingContactsDirectoryVisibility = false
        }

        let previousState = state.hideFromContactsDirectory
        hasChangedContactsDirectoryVisibility = true
        state.isWaitingContactsDirectoryVisibility = true
        state.hideFromContactsDirectory = hidden

        switch await clientProxy.setJunchatHideFromContactsDirectory(hidden) {
        case .success:
            break
        case .failure:
            state.hideFromContactsDirectory = previousState
        }
    }
}
