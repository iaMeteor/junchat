...
struct LoginScreen: View {
    ...
    var header: some View {
        VStack(spacing: 12) {
            AuthenticationStartLogo(size: 88, hideBrandChrome: true, isOnGradient: false)
    ...
    /// Terms and content policy acknowledgement required by App Review for user-generated content apps.
    var termsAgreement: some View {
        VStack(alignment: .leading, spacing: 8) {
            Button {
                context.hasAcceptedContentPolicy.toggle()
            } label: {
                HStack(alignment: .top, spacing: 10) {
                    Image(systemName: context.hasAcceptedContentPolicy ? "checkmark.square.fill" : "square")
                        .font(.body)
                        .foregroundStyle(context.hasAcceptedContentPolicy ? .compound.iconAccentTertiary : .compound.iconSecondary)
                        .frame(width: 22, height: 22)
                    
                    Text("我已阅读并同意《君聊用户协议与内容规范》，并确认不会发布违法、骚扰、仇恨、色情、暴力等不良内容或滥用服务。违规内容和用户可被举报、移除或屏蔽。")
                        .font(.footnote)
            ...
            if let termsURL = URL(string: "https://junchat.yyzs120.cn/junchat-assets/terms.html") {
                Link("查看用户协议与内容规范", destination: termsURL)
                    .font(.footnote.weight(.semibold))
                    .foregroundStyle(.compound.textActionAccent)
            }
        }
        .accessibilityIdentifier("LoginScreen.termsAgreement")
    }
    ...
    private func usernameFocusChanged(isFocussed: Bool) {
        guard context.viewState.allowsAccountProviderChanges, !isFocussed, !context.username.isEmpty else { return }
