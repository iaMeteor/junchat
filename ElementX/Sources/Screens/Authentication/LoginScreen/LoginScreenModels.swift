...
struct LoginScreenViewState: BindableState {
    ...
    /// Whether usernames are allowed to switch the active homeserver.
    var allowsAccountProviderChanges = true
    ...
    var canSubmit: Bool {
        hasValidCredentials && bindings.hasAcceptedContentPolicy && !isLoading
    }
}
...
struct LoginScreenBindings {
    ...
    var password = ""
    /// Whether the user has accepted JunChat's terms and content policy before login.
    var hasAcceptedContentPolicy = false
