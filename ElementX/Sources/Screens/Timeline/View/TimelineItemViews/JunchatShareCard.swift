...
struct JunchatShareCard: Equatable {
    ...
    static func parse(body: String, links: [URL] = []) -> JunchatShareCard? {
        ...
        guard let url = detectedURLs.first ?? links.first(where: \.junchatIsExternalWebURL),
              url.junchatIsExternalWebURL else {
            return nil
        }

        let urlStrings = detectedURLs.map(\.absoluteString) + [url.absoluteString]
        ...
        let shouldReplaceBody = bodyWithoutURLs.isEmpty || (body.count <= 800 && lines.count <= 4)
    ...
    private static func detectURLs(in text: String) -> [URL] {
        ...
        return detector.matches(in: text, options: [], range: nsRange)
            .compactMap(\.url)
            .filter(\.junchatIsExternalWebURL)
    }
