...
private struct TimelineItemSendInfoModifier: ViewModifier {
    ...
    var resolvedLayoutType: TimelineItemSendInfo.LayoutType {
        if dynamicTypeSize.junchatUsesSeparateTimelineSendInfo,
           case .overlay(capsuleStyle: false) = sendInfo.layoutType {
            return .vertical(spacing: 2)
        }
        
        return sendInfo.layoutType
    }
    ...
    func body(content: Content) -> some View {
        layout {
            ...
            TimelineItemSendInfoLabel(sendInfo: sendInfo, layoutType: resolvedLayoutType)
                .contentShape(.rect)
...
private struct TimelineItemSendInfoLabel: View {
    let sendInfo: TimelineItemSendInfo
    let layoutType: TimelineItemSendInfo.LayoutType
    
    init(sendInfo: TimelineItemSendInfo, layoutType: TimelineItemSendInfo.LayoutType? = nil) {
        self.sendInfo = sendInfo
        self.layoutType = layoutType ?? sendInfo.layoutType
    }
