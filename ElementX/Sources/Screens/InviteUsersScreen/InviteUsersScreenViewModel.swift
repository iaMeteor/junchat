...
class InviteUsersScreenViewModel: InviteUsersScreenViewModelType, InviteUsersScreenViewModelProtocol {
    ...
    init(userSession: UserSessionProtocol,
        ...
        Task {
            switch await ContactsService(clientProxy: clientProxy).contacts() {
            case .success(let contacts) where !contacts.isEmpty:
    ...
    private func fetchUsers() {
        ...
        fetchUsersTask = Task {
            if !suggestedUsers.isEmpty {
                let users = suggestedUsers.filter { user in
                    user.matchesJunchatLocalSearch(query: searchQuery)
                }
...
private extension UserProfileProxy {
    func matchesJunchatLocalSearch(query: String) -> Bool {
        let normalizedQuery = query.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        guard !normalizedQuery.isEmpty else {
            return true
        }

        return [displayName, userID, localpart]
