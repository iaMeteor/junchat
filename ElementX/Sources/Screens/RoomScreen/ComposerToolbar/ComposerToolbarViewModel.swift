...
final class ComposerToolbarViewModel: ComposerToolbarViewModelType, ComposerToolbarViewModelProtocol {
        ...
        case .sendMessage:
            ...
            default:
                if context.composerFormattingEnabled {
                    sendMessageAfterContentCheck(plain: wysiwygViewModel.content.markdown,
    ...
    private func sendPlainComposerText() {
        ...
        sendMessageAfterContentCheck(plain: plainComposerContent.text,
                                     html: nil,
                                     mode: state.composerMode,
                                     intentionalMentions: .init(userIDs: plainComposerContent.mentionedUserIDs, atRoom: plainComposerContent.containsAtRoomMention))
    }
    
    private func sendMessageAfterContentCheck(plain: String, html: String?, mode: ComposerMode, intentionalMentions: IntentionalMentions) {
        guard !JunchatContentFilter.containsObjectionableContent(plain) else {
            state.bindings.alertInfo = AlertInfo(id: UUID(),
                                                 title: "内容已拦截",
                                                 message: "此消息可能包含违反君聊用户协议与内容规范的内容，已被本地过滤。请修改后再发送。")
            return
        }
    ...
    private func insertEmoji(_ emoji: String) {
        state.bindings.composerFocused = true
        if context.composerFormattingEnabled {
            let markdown = wysiwygViewModel.content.markdown + emoji
            wysiwygViewModel.setMarkdownContent(markdown)
        } else {
            let mutableText = NSMutableAttributedString(attributedString: state.bindings.plainComposerText)
            let selectedRange = state.bindings.selectedRange
            let insertionLocation: Int
            let replacementLength: Int
            if selectedRange.location != NSNotFound,
               selectedRange.location <= mutableText.length {
                insertionLocation = selectedRange.location
                replacementLength = min(selectedRange.length, mutableText.length - selectedRange.location)
            } else {
                insertionLocation = mutableText.length
                replacementLength = 0
            }
            mutableText.replaceCharacters(in: NSRange(location: insertionLocation, length: replacementLength),
                                          with: NSAttributedString(string: emoji))
            state.bindings.plainComposerText = mutableText
            let cursorLocation = insertionLocation + (emoji as NSString).length
            state.bindings.selectedRange = NSRange(location: cursorLocation, length: 0)
        }
    }
