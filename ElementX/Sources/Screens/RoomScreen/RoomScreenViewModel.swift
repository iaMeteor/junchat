...
class RoomScreenViewModel: RoomScreenViewModelType, RoomScreenViewModelProtocol {
    ...
    init(userSession: UserSessionProtocol,
         ...
         elementCallService: ElementCallServiceProtocol? = nil,
        ...
        let initialRoomHasJoinableCall = roomProxy.infoPublisher.value.hasRoomCall && !roomProxy.infoPublisher.value.activeRoomCallParticipants.isEmpty
        let viewState = RoomScreenViewState(roomTitle: roomProxy.infoPublisher.value.displayName ?? roomProxy.id,
                                            ...
                                            hasOngoingCall: initialRoomHasJoinableCall,
    ...
    override func process(viewAction: RoomScreenViewAction) {
        ...
        case .togglePrivacyMode:
            togglePrivacyMode()
        case .declineCallInvitation:
            state.hasDismissedActiveCallInvitation = true
            Task { await elementCallService?.declineIncomingCall(roomID: roomProxy.id) }
    ...
    private func updateRoomInfo(_ roomInfo: RoomInfoProxyProtocol) {
        let hadOngoingCall = state.hasOngoingCall
        let roomHasJoinableCall = roomInfo.hasRoomCall && !roomInfo.activeRoomCallParticipants.isEmpty

        state.roomTitle = roomInfo.displayName ?? roomProxy.id
        state.roomAvatar = roomInfo.avatar
        state.hasOngoingCall = roomHasJoinableCall
        state.activeRoomCallIntent = roomInfo.activeRoomCallIntent
        state.hasSuccessor = roomInfo.successor != nil

        if hadOngoingCall && !roomHasJoinableCall {
            state.hasDismissedActiveCallInvitation = false
        }
