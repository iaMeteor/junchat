...
private struct CallView: UIViewRepresentable {
    ...
    class Coordinator: NSObject, WKUIDelegate, WKNavigationDelegate, AVPictureInPictureControllerDelegate {
        private enum PictureInPictureDelegateEvent {
            ...
            case willStop(isActive: Bool, isSuspended: Bool)
            case didStop
        }
        ...
        init(viewModelContext: CallScreenViewModel.Context) {
            ...
            pictureInPictureDelegateEventProcessor.start { [weak self] event in
                await self?.processPictureInPictureDelegateEvent(event)
            }
        ...
        func invalidate() {
            ...
            pictureInPictureWebModeTask?.cancel()
        ...
        func stopPictureInPicture() {
            ...
            pictureInPictureRequestTracker.stopRequested()
            pictureInPictureController?.stopPictureInPicture()
        }
        ...
        nonisolated func pictureInPictureControllerWillStartPictureInPicture(_ pictureInPictureController: AVPictureInPictureController) {
            pictureInPictureDelegateEventProcessor.send(.willStart)
        }

        nonisolated func pictureInPictureControllerDidStartPictureInPicture(_ pictureInPictureController: AVPictureInPictureController) {
            pictureInPictureDelegateEventProcessor.send(.didStart(isActive: pictureInPictureController.isPictureInPictureActive,
                                                                  isSuspended: pictureInPictureController.isPictureInPictureSuspended))
        }
        ...
        nonisolated func pictureInPictureControllerWillStopPictureInPicture(_ pictureInPictureController: AVPictureInPictureController) {
            pictureInPictureDelegateEventProcessor.send(.willStop(isActive: pictureInPictureController.isPictureInPictureActive,
                                                                  isSuspended: pictureInPictureController.isPictureInPictureSuspended))
        }

        nonisolated func pictureInPictureControllerDidStopPictureInPicture(_ pictureInPictureController: AVPictureInPictureController) {
            pictureInPictureDelegateEventProcessor.send(.didStop)
        }

        private func processPictureInPictureDelegateEvent(_ event: PictureInPictureDelegateEvent) async {
            guard !isInvalidated else { return }
            ...
            case .didStop:
                ...
                pictureInPictureValidationTask?.cancel()
                pictureInPictureValidationTask = nil
                pictureInPictureRequestTracker.didStop()
                restoreWebViewAfterPictureInPicture()
            }
        }

        private func validateStartedPictureInPicture() {
            pictureInPictureValidationTask?.cancel()
            pictureInPictureValidationTask = Task { @MainActor [weak self] in
                guard let self, !isInvalidated else { return }
                // This is necessary as the page isn't checked when backgrounding the app or tapping a notification.
        ...
        private func restoreWebViewAfterPictureInPicture() {
            guard !isInvalidated else { return }
            webViewWrapper.addMatchedSubview(webView)
        ...
        private func setWebPictureInPictureEnabled(_ enabled: Bool) {
            pictureInPictureWebModeTask?.cancel()
            pictureInPictureWebModeTask = Task { @MainActor [weak self] in
                guard let self, !isInvalidated else { return }
                let function = enabled ? "enablePip" : "disablePip"
                _ = try? await evaluateJavaScript("controls.\(function)()")
            }
        }
