...
class HomeScreenViewModel: HomeScreenViewModelType, HomeScreenViewModelProtocol {
    ...
    init(userSession: UserSessionProtocol,
        ...
        userSession.sessionSecurityStatePublisher
            ...
            .sink { [weak self] securityState in
                ...
                case .disabled:
                    ...
                    if appSettings.hasDismissedChatBackupBanner {
                        state.securityBannerMode = .none
                    } else if !state.securityBannerMode.isDismissed {
                        state.securityBannerMode = .show(.setUpRecovery)
                    }
                case .incomplete:
                    state.requiresExtraAccountSetup = true
                    state.securityBannerMode = appSettings.hasDismissedChatBackupBanner ? .none : .show(.recoveryOutOfSync)
                default:
                    state.securityBannerMode = .none
        ...
        appSettings.$junchatEmergencyPrivacyModeEnabled
            ...
            .sink { [weak self] enabled in
                guard let self else { return }
                state.isEmergencyPrivacyModeEnabled = enabled
                updateRooms()
            }
            .store(in: &cancellables)
    ...
    override func process(viewAction: HomeScreenViewAction) {
        ...
        case .dismissChatBackupBannerPermanently:
    ...
    private func updateRooms() {
        ...
        for summary in roomSummaryProvider.roomListPublisher.value {
            let room = HomeScreenRoom(summary: summary,
                                      ...
                                      hideLastMessage: appSettings.junchatEmergencyPrivacyModeEnabled,
