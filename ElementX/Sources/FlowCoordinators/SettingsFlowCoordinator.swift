...
class SettingsFlowCoordinator: FlowCoordinatorProtocol {
    ...
    // periphery:ignore - retaining purpose
    private var changePasswordScreenCoordinator: ChangePasswordScreenCoordinator?
    // periphery:ignore - retaining purpose
    private var callRingtoneSettingsScreenCoordinator: CallRingtoneSettingsScreenCoordinator?
    // periphery:ignore - retaining purpose
    private var messageNotificationSoundSettingsScreenCoordinator: MessageNotificationSoundSettingsScreenCoordinator?
        ...
        settingsScreenCoordinator.actions
            .sink { [weak self] action in
                ...
                case .labs:
                    presentLabs()
                case .junchatSupport:
                    presentJunchatSupport()
                case .changePassword:
                    presentChangePassword()
                case .callRingtoneSettings:
                    presentCallRingtoneSettings(context: settingsScreenCoordinator.context)
                case .messageNotificationSoundSettings:
                    presentMessageNotificationSoundSettings(context: settingsScreenCoordinator.context)
    ...
    private func presentJunchatSupport() {
        navigationStackCoordinator.push(JunchatSupportScreenCoordinator())
    }

    private func presentChangePassword() {
        let coordinator = ChangePasswordScreenCoordinator(parameters: .init(clientProxy: flowParameters.userSession.clientProxy,
                                                                            userIndicatorController: flowParameters.userIndicatorController))
        coordinator.actionsPublisher
            .sink { [weak self] action in
                ...
                case .passwordChanged:
                    ...
                    self?.changePasswordScreenCoordinator = nil
                }
            }
            .store(in: &cancellables)

        changePasswordScreenCoordinator = coordinator
        coordinator.start()
        navigationStackCoordinator.push(coordinator)
    }

    private func presentCallRingtoneSettings(context: SettingsScreenViewModel.Context) {
        let coordinator = CallRingtoneSettingsScreenCoordinator(context: context)
        callRingtoneSettingsScreenCoordinator = coordinator
        navigationStackCoordinator.push(coordinator)
    }

    private func presentMessageNotificationSoundSettings(context: SettingsScreenViewModel.Context) {
        let coordinator = MessageNotificationSoundSettingsScreenCoordinator(context: context)
        messageNotificationSoundSettingsScreenCoordinator = coordinator
        navigationStackCoordinator.push(coordinator)
    }
