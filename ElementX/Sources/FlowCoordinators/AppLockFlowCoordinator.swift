...
enum AppLockFlowCoordinatorAction: Equatable {
    ...
    case unlockApp
    /// Disable the current account and force a logout.
...
class AppLockFlowCoordinator: CoordinatorProtocol {
    ...
    private var unlockedWithEmergencyPrivacyPIN = false
    ...
    private func configureStateMachine() {
        ...
        stateMachine.addAnyHandler(.any => .any) { [weak self] context in
            ...
            case (_, .attemptingPINUnlock):
                showUnlockScreen()
            case (_, .unlocked):
                if !unlockedWithEmergencyPrivacyPIN {
                    appSettings.junchatEmergencyPrivacyModeEnabled = false
                }
                unlockedWithEmergencyPrivacyPIN = false
    ...
    private func showUnlockScreen() {
        ...
        coordinator.actions.sink { [weak self] action in
            ...
            case .emergencyAccountDisableRequested:
                actionsSubject.send(.disableAccountAndForceLogout)
            case .forceLogout:
