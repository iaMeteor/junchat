...
struct NotificationContentBuilder {
    ...
    let callNotificationSoundName: UNNotificationSoundName?
    ...
    func process(notificationContent: inout UNMutableNotificationContent,
        ...
        notificationContent.sound = notificationSound(for: notificationItem)
    ...
    private func notificationSound(for notificationItem: NotificationItemProxyProtocol) -> UNNotificationSound {
        switch notificationItem.event {
        case .timeline(let event):
            guard case let .messageLike(messageContent) = try? event.content() else {
                return .init(named: notificationSoundName)
            }
            
            switch messageContent {
            case .callInvite, .rtcNotification:
                return callNotificationSound()
            default:
                return .init(named: notificationSoundName)
            }
        default:
    ...
    private func callNotificationSound() -> UNNotificationSound {
        guard let callNotificationSoundName else {
            return .default
        }

        return .init(named: callNotificationSoundName)
    }
