...
With no candidate options, the command verifies that `project.yml` uses the release-locked package at `Vendor/EmbeddedElementCall`, re-hashes that complete package against `ElementCall.release.json`, and then exits. This vendored package is the dependency used by normal, release, and signing workflows. The lock records the promoted candidate manifest digest, source repository commit and tree, package tree digest, and the single RTC configuration authority. The SourcePackages seed is required only when all three candidate identity options are present and is rejected otherwise.
