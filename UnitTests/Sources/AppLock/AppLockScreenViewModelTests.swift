...
final class AppLockScreenViewModelTests {
    ...
    @Test
    func emergencyPrivacyPINRequestsAccountDisableWithoutFailedAttempt() async throws {
        // Given a normal PIN code that is not the emergency PIN.
        keychainController.pinCodeReturnValue = "2023"
        keychainController.containsPINCodeBiometricStateReturnValue = false
        #expect(context.viewState.numberOfPINAttempts == 0)

        // When entering the emergency PIN.
        let deferred = deferFulfillment(viewModel.actions) { $0 == .emergencyAccountDisableRequested }
        viewModel.context.pinCode = "7878"
        let result = try await deferred.fulfill()

        // Then the app should disable the account without counting as a failed attempt.
        #expect(result == .emergencyAccountDisableRequested)
        #expect(context.viewState.numberOfPINAttempts == 0)
    }
