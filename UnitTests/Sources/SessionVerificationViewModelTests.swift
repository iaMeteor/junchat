...
struct SessionVerificationViewModelTests {
    ...
    init() throws {
        sessionVerificationController = SessionVerificationControllerProxyMock.configureMock(requestDelay: .milliseconds(250))
    ...
    mutating func acceptChallenge() async throws {
        ...
        let deferred = deferFulfillment(sessionVerificationController.actions
            .delay(for: .milliseconds(10), scheduler: DispatchQueue.main)) { callback in
    ...
    private mutating func setupChallengeReceived() async throws {
        let actionsPublisher = sessionVerificationController.actions.delay(for: .milliseconds(10), scheduler: DispatchQueue.main)
