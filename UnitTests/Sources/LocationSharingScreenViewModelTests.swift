...
struct LocationSharingScreenViewModelTests {
    ...
    @Test
    mutating func startLiveLocationWithWhenInUseAuthorizationShowsDisclaimer() {
    ...
    @Test
    mutating func startLiveLocationWithWhenInUseAuthorizationDoesNotRequestAlwaysAuthorization() {
        let liveLocationManagerMock = LiveLocationManagerMock(.init(authorizationStatus: .authorizedWhenInUse))
    ...
    mutating func startLiveLocationWithNotDeterminedAuthorizationTransitionsToWhenInUse() async {
        ...
        let liveLocationManagerMock = LiveLocationManagerMock(.init())
        liveLocationManagerMock.underlyingAuthorizationStatus = .init(authorizationStatusSubject)
        setupViewModel(liveLocationManagerMock: liveLocationManagerMock)
        
        context.send(viewAction: .startLiveLocation)
        
        // No alert yet — waiting for the map view to resolve the status to whenInUse.
        #expect(context.alertInfo == nil)
        
        let deferred = deferFulfillment(context.observe(\.alertInfo)) { $0?.id == .liveLocationDisclaimer }
    ...
    mutating func viewLiveFromBannerAwaitsFirstShareThenCentersOnIt() async throws {
        ...
        // Initially no annotations, but the map has a stable fallback center until the first share arrives.
        #expect(context.viewState.annotations.isEmpty)
        #expect(context.mapCenterLocation == context.viewState.initialMapCenter)
...
@MainActor
struct JunchatMapKitMapViewTests {
    @Test
    func updateUserLocationModeDoesNotQueryAuthorizationOnEverySwiftUIUpdate() {
        let locationManager = JunchatMapKitLocationManagerMock(authorizationStatus: .authorizedWhenInUse)
        let mapView = MKMapView(frame: .zero)
        let coordinator = JunchatMapKitMapView.Coordinator(makeMapView(showing: .show), locationManager: locationManager)
        
        #expect(locationManager.authorizationStatusCallCount == 1)
        
        coordinator.updateUserLocationMode(on: mapView)
        coordinator.updateUserLocationMode(on: mapView)
    ...
    @Test
    func updateUserLocationModeRequestsLocationAuthorizationOnlyOnce() {
        let locationManager = JunchatMapKitLocationManagerMock(authorizationStatus: .notDetermined)
        let mapView = MKMapView(frame: .zero)
        let coordinator = JunchatMapKitMapView.Coordinator(makeMapView(showing: .showAndFollow), locationManager: locationManager)
    ...
    @Test
    func updateUserLocationModeDoesNotReadLocationSynchronouslyWithoutDelegateLocation() {
        let locationManager = JunchatMapKitLocationManagerMock(authorizationStatus: .authorizedWhenInUse)
        let mapView = MKMapView(frame: .zero)
        let coordinator = JunchatMapKitMapView.Coordinator(makeMapView(showing: .showAndFollow), locationManager: locationManager)
        ...
        #expect(locationManager.locationCallCount == 0)
    }
    
    @Test
    func updateUserLocationModeStopsLoadingIfInitialLocationDoesNotArrive() async throws {
        let locationManager = JunchatMapKitLocationManagerMock(authorizationStatus: .authorizedWhenInUse)
        let mapView = MKMapView(frame: .zero)
        let mode = BindingBox<ShowUserLocationMode>(.showAndFollow)
        let hasLoadedUserLocation = BindingBox(false)
        let coordinator = JunchatMapKitMapView.Coordinator(makeMapView(showing: mode,
                                                                       hasLoadedUserLocation: hasLoadedUserLocation),
                                                           locationManager: locationManager,
                                                           locationFallbackDelay: 0.01)
        
        coordinator.updateUserLocationMode(on: mapView)
        
        try await Task.sleep(for: .milliseconds(50))
        
        #expect(hasLoadedUserLocation.value)
        #expect(mode.value == .show)
    }
    
    @Test
    func updateUserLocationModeDoesNotRecenterSameUserLocationOnEverySwiftUIUpdate() {
        let locationManager = JunchatMapKitLocationManagerMock(authorizationStatus: .authorizedWhenInUse)
        let mapView = MKMapView(frame: .zero)
        let center = BindingBox<CLLocationCoordinate2D?>(nil)
        let coordinator = JunchatMapKitMapView.Coordinator(makeMapView(showing: BindingBox(.showAndFollow),
                                                                       mapCenter: center,
                                                                       hasLoadedUserLocation: BindingBox(false)),
                                                           locationManager: locationManager,
                                                           locationFallbackDelay: 0)
        
        coordinator.updateUserLocationMode(on: mapView)
        coordinator.locationManager(CLLocationManager(), didUpdateLocations: [CLLocation(latitude: 23.1291, longitude: 113.2644)])
        let setCountAfterFirstLocation = center.setCount
        
        coordinator.updateUserLocationMode(on: mapView)
        coordinator.updateUserLocationMode(on: mapView)
        
        #expect(center.setCount == setCountAfterFirstLocation)
    }
    
    private func makeMapView(showing mode: ShowUserLocationMode) -> JunchatMapKitMapView {
        makeMapView(showing: BindingBox(mode), hasLoadedUserLocation: BindingBox(false))
    }
    
    private func makeMapView(showing mode: BindingBox<ShowUserLocationMode>,
                             hasLoadedUserLocation: BindingBox<Bool>) -> JunchatMapKitMapView {
        makeMapView(showing: mode, mapCenter: BindingBox<CLLocationCoordinate2D?>(nil), hasLoadedUserLocation: hasLoadedUserLocation)
    }
    
    private func makeMapView(showing mode: BindingBox<ShowUserLocationMode>,
                             mapCenter center: BindingBox<CLLocationCoordinate2D?>,
                             hasLoadedUserLocation: BindingBox<Bool>) -> JunchatMapKitMapView {
        let isLocationAuthorized = BindingBox<Bool?>(nil)
...
private final class JunchatMapKitLocationManagerMock: JunchatMapKitLocationManagerProtocol {
    weak var delegate: CLLocationManagerDelegate?
    
    private let underlyingAuthorizationStatus: CLAuthorizationStatus
    private let underlyingLocation: CLLocation?
    
    private(set) var authorizationStatusCallCount = 0
    private(set) var locationCallCount = 0
    ...
    var authorizationStatus: CLAuthorizationStatus {
        ...
        return underlyingAuthorizationStatus
    }
    ...
    init(authorizationStatus: CLAuthorizationStatus, location: CLLocation? = nil) {
        underlyingAuthorizationStatus = authorizationStatus
