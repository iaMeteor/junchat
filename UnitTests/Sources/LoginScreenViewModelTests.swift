...
struct LoginScreenViewModelTests {
    ...
    @Test
    mutating func validCredentialsRequireContentPolicyAcceptance() async {
        // Given a form with an empty username and password.
        await setupViewModel()
        #expect(context.password.isEmpty,
                "The initial value for the password should be empty.")
        ...
        // Then the credentials should be considered valid, but the form should remain blocked until the content policy is accepted.
        #expect(context.viewState.hasValidCredentials,
                "The credentials should be valid when the username and password are valid.")
        #expect(!context.viewState.canSubmit,
                "The form should remain blocked until the content policy is accepted.")
        
        // When accepting the content policy.
        context.hasAcceptedContentPolicy = true
        
        // Then the form should be ready to submit.
        #expect(context.viewState.canSubmit,
                "The form should be ready to submit after accepting the content policy.")
    }
