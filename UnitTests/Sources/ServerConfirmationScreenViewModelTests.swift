...
final class ServerConfirmationScreenViewModelTests {
    ...
    func elementProRequired() async throws {
        ...
        #expect(clientFactory.makeClientHomeserverAddressSessionDirectoriesPassphraseClientSessionDelegateAppSettingsAppHooksCallsCount == 1)
        let expectedServerName = LoginHomeserver(address: client.server() ?? client.homeserver(), loginMode: .unknown).address
        #expect(context.alertInfo?.id == .elementProRequired(serverName: expectedServerName))
    }
    ...
    private func setupViewModel(authenticationFlow: AuthenticationFlow,
        ...
        let defaultAccountProvider = appSettings.accountProviders[0]
        var mode = ServerConfirmationScreenMode.confirmation(defaultAccountProvider)
        if restrictedFlow {
        ...
        var clientConfiguration = ClientSDKMock.Configuration()
        clientConfiguration.serverAddress = restrictedFlow ? "matrix.org" : defaultAccountProvider
        clientConfiguration.oidcLoginURL = supportsOIDC ? "https://account.matrix.org/authorize" : nil
        clientConfiguration.supportsOIDCCreatePrompt = supportsOIDCCreatePrompt
        clientConfiguration.supportsPasswordLogin = supportsPasswordLogin
        clientConfiguration.elementWellKnown = requiresElementPro ? "{\"version\":1,\"enforce_element_pro\":true}" : nil
