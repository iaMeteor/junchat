...
struct NavigationTabCoordinatorTests {
    ...
    mutating func overlay() {
        ...
        assertCoordinatorsEqual(overlayCoordinator, navigationTabCoordinator.overlayCoordinator)
        #expect(!navigationTabCoordinator.overlayAllowsHitTesting)
        ...
        #expect(overlayCoordinator.stopCallCount == 1)
    }

    @Test
    mutating func replacingOverlayStopsPreviousCoordinator() {
        let firstOverlayCoordinator = SomeTestCoordinator()
        let secondOverlayCoordinator = SomeTestCoordinator()

        navigationTabCoordinator.setOverlayCoordinator(firstOverlayCoordinator)
        navigationTabCoordinator.setOverlayCoordinator(secondOverlayCoordinator)

        #expect(firstOverlayCoordinator.stopCallCount == 1)
        #expect(secondOverlayCoordinator.stopCallCount == 0)
        assertCoordinatorsEqual(secondOverlayCoordinator, navigationTabCoordinator.overlayCoordinator)
    }
    ...
    @Test
    mutating func tabDismissalCallbacksNotCalledForReusedTabs() async {
        let chatsCoordinator = SomeTestCoordinator()
        let contactsCoordinator = SomeTestCoordinator()
        let spacesCoordinator = SomeTestCoordinator()

        await waitForConfirmation("Callbacks should not be called for reused tab coordinators",
                                  expectedCount: 0,
                                  timeout: .seconds(1)) { confirmation in
            navigationTabCoordinator.setTabs([
                .init(coordinator: chatsCoordinator, details: .init(tag: .chats, title: "Chats", icon: \.chat, selectedIcon: \.chatSolid)) { confirmation() },
                .init(coordinator: spacesCoordinator, details: .init(tag: .spaces, title: "Spaces", icon: \.space, selectedIcon: \.spaceSolid)) { confirmation() }
            ])

            navigationTabCoordinator.setTabs([
                .init(coordinator: chatsCoordinator, details: .init(tag: .chats, title: "Chats", icon: \.chat, selectedIcon: \.chatSolid)) { confirmation() },
                .init(coordinator: contactsCoordinator, details: .init(tag: .tab, title: "Contacts", icon: \.userProfile, selectedIcon: \.userProfileSolid)),
                .init(coordinator: spacesCoordinator, details: .init(tag: .spaces, title: "Spaces", icon: \.space, selectedIcon: \.spaceSolid)) { confirmation() }
            ])
        }
    }
