...
final class HomeScreenViewModelTests {
    ...
    @Test
    func permanentlyDismissedChatBackupBannerDoesNotShowAfterHomeIsRecreated() async throws {
        appSettings.hasDismissedChatBackupBanner = true
        let securityStateStateSubject = CurrentValueSubject<SessionSecurityState, Never>(.init(verificationState: .verified, recoveryState: .unknown))

        setupViewModel(securityStatePublisher: securityStateStateSubject.asCurrentValuePublisher())

        let deferred = deferFulfillment(context.$viewState) { $0.requiresExtraAccountSetup == true }
        securityStateStateSubject.send(.init(verificationState: .verified, recoveryState: .disabled))
        try await deferred.fulfill()

        #expect(context.viewState.requiresExtraAccountSetup)
        #expect(context.viewState.securityBannerMode == .none)
    }

    @Test
    func permanentlyDismissChatBackupBannerActionPersistsTheChoice() async throws {
        let securityStateStateSubject = CurrentValueSubject<SessionSecurityState, Never>(.init(verificationState: .verified, recoveryState: .unknown))
        setupViewModel(securityStatePublisher: securityStateStateSubject.asCurrentValuePublisher())

        let deferred = deferFulfillment(context.$viewState) { $0.securityBannerMode == .show(.setUpRecovery) }
        securityStateStateSubject.send(.init(verificationState: .verified, recoveryState: .disabled))
        try await deferred.fulfill()

        #expect(context.viewState.securityBannerMode == .show(.setUpRecovery))

        context.send(viewAction: .dismissChatBackupBannerPermanently)
