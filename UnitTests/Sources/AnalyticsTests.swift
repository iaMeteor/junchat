...
final class AnalyticsTests {
    ...
    private let testAnalyticsConfiguration = AnalyticsConfiguration(host: "https://posthog.example.com",
    ...
    func analyticsPromptNewUser() {
        // Given a fresh install of JunChat where tracking prompts are disabled.
        // When the user is prompted for analytics.
        let showPrompt = ServiceLocator.shared.analytics.shouldShowAnalyticsPrompt
        
        // Then the prompt should not be shown.
        #expect(!showPrompt, "A tracking prompt should not be shown for JunChat.")
    }
    ...
    @Test
    func analyticsOptIn() {
        // Given a fresh install of JunChat with analytics disabled.
        // When analytics is opt-in
        ServiceLocator.shared.analytics.optIn()
        // The consent state may be stored, but no analytics client should start.
    ...
    func analyticsStartIfEnabled() {
        // Given an existing install of JunChat where analytics was previously accepted.
        appSettings.analyticsConsentState = .optedIn
        // Analytics should still stay stopped because JunChat doesn't ship a tracking configuration.
        #expect(ServiceLocator.shared.analytics.isEnabled)
        ServiceLocator.shared.analytics.startIfEnabled()
        #expect(!analyticsClient.startAnalyticsConfigurationCalled)
    }
    ...
    func sendingUserProperties() {
        ...
        client.start(analyticsConfiguration: testAnalyticsConfiguration)
    ...
    func resetConsentState() {
        ...
        // Then the analytics prompt should remain hidden.
        #expect(appSettings.analyticsConsentState == .unknown)
        #expect(!ServiceLocator.shared.analytics.shouldShowAnalyticsPrompt)
    }
