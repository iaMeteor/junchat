...
struct EmojiPickerScreenViewModelTests {
    ...
    @Test
    mutating func composerSelectionReturnsEmojiAndDismisses() async throws {
        let reaction = "😀"
        var selectedEmoji: String?
        viewModel = EmojiPickerScreenViewModel(emojiProvider: EmojiProvider(appSettings: ServiceLocator.shared.settings)) { emoji in
            selectedEmoji = emoji
        }
        
        let deferred = deferFulfillment(viewModel.actions) { $0 == .dismiss }
        
        context.send(viewAction: .emojiTapped(emoji: .init(id: "grinning", value: reaction)))
        
        try await deferred.fulfill()
        #expect(selectedEmoji == reaction)
    }
