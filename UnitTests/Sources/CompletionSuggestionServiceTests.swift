...
struct CompletionSuggestionServiceTests {
    ...
    func userSuggestions() async throws {
        ...
        let service = CompletionSuggestionService(roomProxy: roomProxyMock,
                                                  ...
                                                  suggestionDebounceDuration: .milliseconds(0))
    ...
    func userSuggestionsIncludingAllUsers() async throws {
        ...
        let everyoneSearchText = L10n.commonEveryone
        deferred = deferFulfillment(service.suggestionsPublisher) { suggestions in
            suggestions == [.init(suggestionType: .allUsers(.room(id: "roomID", name: "test", avatarURL: nil)), range: .init(), rawSuggestionText: everyoneSearchText)]
        }
        service.setSuggestionTrigger(.init(type: .user, text: everyoneSearchText, range: .init()))
        try await deferred.fulfill()
    }
