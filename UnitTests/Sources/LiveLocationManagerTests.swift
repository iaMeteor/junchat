...
final class LiveLocationManagerTests {
    ...
    @Test
    func initDoesNotCreateLocationManagerWhenLocationSharingIsDisabled() {
        appSettings = AppSettings()
        clientProxy = ClientProxyMock(.init())
        
        var locationManagerWasCreated = false
        _ = LiveLocationManager(clientProxy: clientProxy,
                                appSettings: appSettings,
                                locationSharingEnabled: false,
                                locationManager: {
                                    locationManagerWasCreated = true
                                    return CLLocationManagerMock(.init())
                                }())
        
        #expect(locationManagerWasCreated == false)
    }
    
    @Test
    func initDoesNotEnableBackgroundLocationWithoutBackgroundMode() {
        setUp()
        
        #expect(locationManagerMock.underlyingAllowsBackgroundLocationUpdates != true)
        #expect(locationManagerMock.underlyingShowsBackgroundLocationIndicator != true)
    }
    
    @Test
    func initDoesNotCreateDeferredLocationManagerWhenLocationSharingIsEnabled() {
        appSettings = AppSettings()
        clientProxy = ClientProxyMock(.init())
        
        var locationManagerWasCreated = false
        let deferredLocationManager = DeferredCLLocationManager {
            locationManagerWasCreated = true
            return CLLocationManagerMock(.init())
        }
        ...
        _ = LiveLocationManager(clientProxy: clientProxy,
                                ...
                                locationManager: deferredLocationManager)
        
        #expect(locationManagerWasCreated == false)
    }
    
    @Test
    func startLiveLocationCreatesDeferredLocationManagerWhenSessionStarts() async throws {
        appSettings = AppSettings()
        clientProxy = ClientProxyMock(.init())
        ...
        var locationManagerWasCreated = false
        let locationManagerMock = CLLocationManagerMock(.init())
        let deferredLocationManager = DeferredCLLocationManager {
            locationManagerWasCreated = true
            return locationManagerMock
        }
        
        manager = LiveLocationManager(clientProxy: clientProxy,
                                      appSettings: appSettings,
                                      locationSharingEnabled: true,
                                      locationManager: deferredLocationManager)
        ...
        try result.get()
        #expect(locationManagerWasCreated)
